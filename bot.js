...
client.on("guildMemberAdd", member => {
  member.roles.add('752193453739343892'); // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
});
...
client.on("guildMemberAdd", member => {  
    const kanal = member.guild.channels.cache.find(r => r.id === "752197112699027547");
...
client.on("userUpdate", async (stg, yeni) => {
  ...
  var tagrol = "752251899717222432"; // Buraya Ekip Rolünün ID
...
client.on('message', lrowstagmesaj => {
  if (lrowstagmesaj.content.toLowerCase() === '!tag') {
    lrowstagmesaj.channel.send('**<a:uyar8:740220446259019836> Hey sen tagımızımı alacaksın işte tagımız ; ✯**');//TAG
  }
});
client.on('message', lrowstagmesaj => {
  if (lrowstagmesaj.content.toLowerCase() === 'tag') {
